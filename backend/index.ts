...
import { fetchServers } from "./services/fivem.service";

import * as Server from "./services/server.service";
import * as ServerHistory from "./services/server.history.service";

const MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost:27017/fivem-tracker";
...
app.get("/server/:id/history/:period", (req: any, res: any) => {
  ...
  ServerHistory.get(req.params.id, req.params.period).then((history) => {
    res.send(history);
  }).catch((error) => {
